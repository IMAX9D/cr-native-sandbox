"""One-command decision training: prepare missing cache and resume last.pt automatically.

Defaults match ~/cr-data. Each invocation adds up to 1000 updates (within the
configured epoch limit); --max-steps overrides this with an absolute limit.
"""
...
from hokoff_model.train_decisions import parser as trainer_parser, run
...
def parser():
    ...
    p.add_argument('--steps', type=int, default=1000, help='additional updates this invocation, unless --max-steps is set')
...
def main(argv=None):
    ...
    if args.resume is not None:
        ...
        if saved.get('config', {}).get('architecture') != 'hokoff_cr_lstm_decisions_v1':
            p.error('resume requires a decision-model checkpoint')
        completed_step = int(saved['step'])
        del saved
    if args.max_steps is None:
        args.max_steps = completed_step+steps
    cache_missing = not (args.cache/'index.json').is_file()
    print(json.dumps({'phase':'decision_launcher', 'prepare_cache':cache_missing,
