...
项目介绍视频的[口播稿与拍摄资料](docs/media/README.md)已整理，涵盖原生接口、指针链、Linux 运行与训练状态边界。
...
| MuMu ARM64 实时接口 | `160402002` + 资源 `16.402.7` | 同帧读取、双击下牌与回执已实测；仍严格锁定 libg SHA |
...
实时接口已完成同一 Tick 的实体/手牌/圣水读取、己方识别、重绑定、普通双击下牌及回执验收。
最终校准运行发送 6 次、确认 6 次，两个静止建筑落点与请求格子一致，终局后自动停手。
这不是模型实力测试，也不是新版 ARM64 无界面宿主的移植。

先按 [MuMu 部署与验收入口](docs/MUMU_LIVE_QUICKSTART.zh-CN.md) 安装读取器，再运行不需要模型的只读探针。
实际下牌测试必须显式加 `--execute`；最多一场、六次下牌，失败不盲目补发。
可选的专家接管仍需要你提供兼容权重与词表，仓库不附带模型。

`libg.so` 只读，动作通过普通 Android 触控发送；不修改游戏内存、不调用客户端内部命令。
版本、ABI 或 SHA 不符会拒绝运行。详见 [指针链与实测记录](docs/MUMU_LIVE_POINTER_ADAPTATION_20260908.zh-CN.md)。
