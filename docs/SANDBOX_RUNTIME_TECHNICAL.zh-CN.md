...
| 觉醒循环和英雄/冠军主动技能 | 管理持久 TCP、进程内重置和 GUI |
...
| 路径 | 作用 | 写入策略 |
| --- | --- | --- |
| `D:\Deepseek\CR-Native-Core` | 当前宿主、JNI、Python API、GUI、文档 | 当前工程可写 |
| `D:\Codex\E\AI ClashRoyale` | 冻结 APK、原生库和旧工程参考 | 只读来源 |
| `D:\Deepseek\cr_re` | 静态逆向、数据表和证据 | 只读来源 |
| `D:\AI_data\cr-native-core` | 证书、日志、GUI 快照和运行时缓存 | 运行时可写 |
...
```mermaid
flowchart LR
    U[GUI / Python 调用方] <-->|持久 JSON-line TCP| J[Java JniHost]
    J <--> B[JNI Bridge]
    B <--> G[原版 libg.so]
...
| `native_core/gui.py` | 逻辑可视化和人工验收 GUI |
...
当前目录由 `scripts/build_live_card_catalog.py` 生成到
`native_core/data/live_card_catalog.json`。来源包括基础卡映射、当前 DataTables
逆向结果、觉醒/英雄 Spell 表和角色 TOML。
...
D:\AI_data\runtime\venv\Scripts\python.exe `
  ...
  --output D:\AI_data\cr-native-core\sandbox-replay.json
```
...
with NativeRoyaleEnv(port=37031) as env:
    state = env.reset(
        Path(r"D:\AI_data\cr-native-core\sandbox-replay.json"),
        warmup_steps=100,
    )
    grid = env.probe_grid(side=0, deck_index=2)
...
原生 validator 提供地形和卡牌约束。GUI 还提供一层宿主侧竞技场边界修正，
用于表达已实测但原生粗网格未完整表达的内容：
...
这层修正不是 `libg` 单个函数的输出。GUI 可切换“原始 libg 掩码”和修正后
边界，用于对照；任何边界变化都应重新做实机与原生差分。
...
```powershell
.\scripts\start_logic_gui.ps1 -Port 37031
```

GUI 当前提供：

- reset、`+1/+20/+200 Tick`、自动推进；
- 红/蓝方选择；
- 当前手牌与卡组；
- 点击地图部署；
- 原始网格/修正边界切换；
- 六塔、河流、桥和实体绘制；
- 当前目标连线与原生路径节点；
- HP、坐标、behavior、攻击和碰撞字段；
- 比赛时钟与 ×1/×2/×3 圣水阶段；
- state hash、RNG、终局和 JSON 快照导出。

当前 GUI 尚未提供主动技能按钮；主动技能通过 Python/JSON API 调用。GUI 默认
加载八卡模板，也可用 `--replay` 指向任意全卡 Replay。
...
.\scripts\start_direct_service.ps1 `
  -Port 37031 `
  -Slot 0 `
  -BootstrapReplayJson D:\AI_data\cr-native-core\sandbox-replay.json
```
