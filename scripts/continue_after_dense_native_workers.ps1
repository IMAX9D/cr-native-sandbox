param(
    [Parameter(Mandatory = $true)] [int]$GeneratorPid,
    [Parameter(Mandatory = $true)] [int]$Workers,
    [Parameter(Mandatory = $true)] [int]$Avds,
    [Parameter(Mandatory = $true)] [int]$WorkersPerAvd
)
...
& $python -m native_core.worker stop --workers $Workers --avds $Avds `
    ...
    Out-File -LiteralPath (Join-Path $logRoot 'native-dense-stop.log') -Encoding utf8
$stopExitCode = $LASTEXITCODE

$payload = [ordered]@{
    schema_version = 1
    kind = 'cr_native_dense_continuation_v1'
...
if ($exitCode -eq 0 -and $stopExitCode -eq 0) {
    $launcher = Start-Process -FilePath 'powershell.exe' -ArgumentList @(
      ...
      -RedirectStandardOutput (Join-Path $logRoot 'post-dense-one-click.stdout.log') `
      -RedirectStandardError (Join-Path $logRoot 'post-dense-one-click.stderr.log') `
      -PassThru
