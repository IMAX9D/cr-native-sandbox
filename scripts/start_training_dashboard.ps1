...
$LogRoot = "D:\AI_data\cr-native-core\selfplay-v0.1\dashboard\launcher-logs"
New-Item -ItemType Directory -Path $LogRoot -Force | Out-Null
$Stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$Stdout = Join-Path $LogRoot "$Stamp-dashboard-stdout.log"
$Stderr = Join-Path $LogRoot "$Stamp-dashboard-stderr.log"
$Arguments = @(
    (Join-Path $PSScriptRoot "run_training_dashboard.py"),
    "--port", "$Port"
)
...
do {
    ...
    if ($Process.HasExited) {
        ...
        throw "Training dashboard exited during startup: $ErrorText"
    }
    try {
...
if ($null -eq $State) {
    throw "Training dashboard did not answer within 20 seconds"
}
[pscustomobject]@{
    pid = $Process.Id
    url = $Url
    phase = $State.phase_label
    native_ticks = $State.native_ticks
