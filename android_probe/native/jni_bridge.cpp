...
static jstring observe_state_json(
    JNIEnv* env, jstring libg_path, bool compact_observation) {
  const char* path_chars = env->GetStringUTFChars(libg_path, nullptr);
  if (path_chars == nullptr) {
    return nullptr;
  }
  ...
  std::string result;
  result.reserve(
      512 + static_cast<size_t>(count) * (compact_observation ? 112 : 256));
  char header[320];
  const char* header_format = compact_observation
      ? "{\"schema_version\":1,\"kind\":\"libg_native_compact_state_v1\","
  ...
  for (int32_t index = 0; index < count; ++index) {
    ...
    std::array<int32_t, kMaxPathNodes> path_nodes = {};
    if (!compact_observation && components[0] != 0) {
    ...
    if (!compact_observation && components[1] != 0) {
...
extern "C" JNIEXPORT jstring JNICALL
Java_royale_nativehost_JniHost_nativeObserveCompact(
    JNIEnv* env, jclass, jstring libg_path) {
