"""External facade for the persistent, Surface-free native ``libg`` service."""
...
try:
    ...
    from deployment import deployment_mask  # type: ignore[no-redef]
...
class NativeRoyaleEnv:
    """Synchronous external API for one Surface-free native battle."""
    ...
    def observe_compact(self) -> dict[str, Any]:
        """Return the versioned compact public observation."""
        state = self._request({"op": "observe_compact_v1"})["state"]
        if (
            state.get("schema_version") != 1
            or state.get("kind") != "libg_native_compact_state_v1"
            ...
            raise NativeHostError("compact observation contract mismatch")
        return self._enrich_state(state)
    ...
    def deployment_grid(
        self, *, side: int, deck_index: int, adjusted: bool = True
    ) -> dict[str, Any]:
        """Return raw libg cells and the optional arena-boundary adjustment."""
        state = self.observe()
        player = next(
            item for item in state["players"] if int(item["side"]) == side
        )
        if deck_index not in player["hand_deck_indices"]:
            raise ValueError("deck_index is not in the current hand")
        raw = self.probe_grid(side=side, deck_index=deck_index)
        card_id = int(self.decks[side][deck_index]["card_id"])
        rows = [str(row) for row in raw["rows"]]
        return {
            **raw,
            "raw_rows": rows,
            "rows": deployment_mask(
                rows, state, side=side, card_id=card_id
            ) if adjusted else rows,
