...
def tier(count,rounds,checkpoint,contract):
    ...
    try:
        for i in range(count):
            ...
            cmd=[sys.executable,str(ROOT/'policy_shared_test.py'),'--port',str(BASE_PORT+i),'--raw','--device','cuda',
                 '--modes',','.join(['packed']*rounds),'--seed',str(424242+i*4),'--seed-stride','1000',
        ...
        result=dict(workers=count,resident_games=count*4,model_copies=1,policy_backend='shared_gpu',rounds=rounds,
                    complete_games=len(rows)*4,wall_seconds=wall,ticks=sum(r['ticks'] for r in rows),
...
def main():
    ...
    for n in map(int,a.counts.split(',')):
        ...
        results.append(tier(n,a.rounds,a.checkpoint,a.contract))
