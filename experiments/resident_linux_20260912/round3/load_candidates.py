...
def native(which,op,count):
    script=Path('/root/autodl-tmp/resident-opt-20260912/launch_opt.py')
...
def run(index,variant,count=12,rounds=1):
    ...
    try:
        for i in range(count):
            ...
            cmd=[sys.executable,str(ROOT/'run_variant.py'),'--port',str(25431+i),
...
if __name__=='__main__':
    ...
    results=[]
    try:
        for i,variant in enumerate(('control','buffers','graph','graph','buffers','control')):
