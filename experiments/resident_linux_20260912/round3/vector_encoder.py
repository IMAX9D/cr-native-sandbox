...
def _grid(actor: ActorTick) -> np.ndarray:
    ...
    if actor.entities:
        # add.at preserves repeated-cell order; do not replace by an unordered sum.
        cells=np.asarray([_cell(e.x,e.y) for e in actor.entities],dtype=np.int64)
        offsets=np.asarray([4 if e.relation==0 else 6 for e in actor.entities],dtype=np.int64)
        hp=np.asarray([max(0.,min(1.,e.hp/e.max_hp))/16. if e.max_hp>0 else 0. for e in actor.entities],dtype=np.float32)
        rows,columns=np.divmod(cells,ARENA_COLUMNS)
        np.add.at(result,(offsets,rows,columns),np.float32(1./16.))
        np.add.at(result,(offsets+1,rows,columns),hp)
    ...
    def encode_batch(
        ...
        *,
        ...
        for index, (frame, state, actor, deck) in enumerate(
            ...
            count=len(entity_rows[index])
            if count:
                categorical=np.asarray([r[:3] for r in entity_rows[index]],dtype=np.int64)
                numeric=np.asarray([r[3] for r in entity_rows[index]],dtype=np.float32)
                entity_tokens[index,0,:count]=categorical[:,0]
                entity_positions[index,0,:count]=categorical[:,1]
                entity_relations[index,0,:count]=categorical[:,2]
                entity_numeric[index,0,:count]=numeric
                entity_mask[index,0,:count]=True
