...
def run(model,encoder,c,args,mode,seed):
    ...
    game_hashes=[hashlib.sha256() for _ in envs];game_plays=[0 for _ in envs]
    for cycle in range(2200):
        ...
        for r in results:
            ...
            game_plays[i]+=len(actions[i]);game_hashes[i].update((json.dumps((before,actions[i]),sort_keys=True)+'\n').encode())
            action_log.append((i,before,actions[i]))
            states[i]=envs[i]._enrich_state(r['state'])
            if done:
                finals[i]=dict(tick=after,outcome=ep['outcome'],crowns=ep['crowns'],plays=game_plays[i],action_digest=game_hashes[i].hexdigest())
                active.remove(i)
...
def main():
    ...
    model,encoder,identity=load_release(a.checkpoint,contract_path=a.contract,device=a.device)
