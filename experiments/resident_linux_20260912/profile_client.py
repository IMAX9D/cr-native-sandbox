...
class JsonLineClient:
    ...
    def request(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        encoded_cpu = time.thread_time()
        encoded = json.dumps(
            payload, ensure_ascii=False, separators=(",", ":")
        ).encode("utf-8") + b"\n"
        serialized = time.perf_counter()
        serialized_cpu = time.thread_time()
        ...
        with self._lock:
            for attempt in range(2):
                ...
                try:
                    ...
                    send_cpu = time.thread_time()
                    self._socket.sendall(encoded)
                    sent = time.perf_counter()
                    sent_cpu = time.thread_time()
                    raw = self._reader.readline(MAX_RESPONSE_BYTES + 2)
                    received = time.perf_counter()
                    received_cpu = time.thread_time()
                    if not raw:
                    ...
                    parsed_cpu = time.thread_time()
                    if not isinstance(response, dict):
                        raise TypeError("native host response root must be an object")
                    if self.profile is not None:
                        values = {
                            "rpc_calls": 1.0,
                            "rpc_serialize_cpu_seconds": serialized_cpu - encoded_cpu,
                            "rpc_send_cpu_seconds": sent_cpu - send_cpu,
                            "rpc_receive_cpu_seconds": received_cpu - sent_cpu,
                            "rpc_parse_cpu_seconds": parsed_cpu - received_cpu,
