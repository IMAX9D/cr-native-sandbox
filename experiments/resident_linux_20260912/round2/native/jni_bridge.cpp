...
#include "resident_declarations.h"
...
bool capture_episode_state(
  ...
  native_profile::Scope timing(native_profile::context==1?native_profile::CAPTURE_STEP:(native_profile::context==2?native_profile::CAPTURE_OBS:native_profile::OTHER_CAPTURE));
  ...
  // Once all six original towers are tracked, unknown entity bodies cannot
  // affect this six-entry terminal snapshot. Still recheck table membership,
  // full tower identity filters and HP through checked reads on every update.
  static const bool tower_capture_fast=[] {
    const char* value=std::getenv("CR_NATIVE_TOWER_CAPTURE_FAST");
    return value && std::strcmp(value,"1")==0;
  }();
  EntityPointerSnapshot entity_pointers(memory, data, count,
      batch_entity_pointers_enabled() || tower_capture_fast);
  record_pointer_table(entity_pointers);
  for (int32_t index = 0; index < count; ++index) {
    uint64_t entity = 0;
    unsigned char raw[0x124] = {};
    if (!entity_pointers.read(index, &entity) || entity == 0) {
      continue;
    }
    if(tower_capture_fast && g_episode.tower_count==6) {
      bool known=false;
      for(size_t t=0;t<g_episode.tower_count;++t) known=known || g_episode.towers[t].id==entity;
      if(!known) continue;
    }
    if(!memory.read_bytes(entity, raw, sizeof(raw))) continue;
...
std::string episode_json() {
  native_profile::Scope timing(native_profile::context==2?native_profile::EPISODE_OBS:native_profile::EPISODE_STEP);
...
Java_royale_nativehost_JniHost_nativeAct(
  ...
  native_profile::Scope timing(native_profile::ACTION);
  ...
  if (!memory.read(base + kManagerGlobalRva, &manager) || manager == 0 ||
      ...
      !resident_read_battle(memory, state, &battle) || battle == 0 ||
...
Java_royale_nativehost_JniHost_nativeProbeGrid(
  ...
  native_profile::Scope timing(native_profile::GRID);
  if (side < 0 || side > 1 || deck_index < 0 || deck_index > 7) {
...
static jstring observe_state_json(
  ...
  native_profile::Scope timing(native_profile::OBSERVE);
  native_profile::Context profile_context(2);
  const char* path_chars = env->GetStringUTFChars(libg_path, nullptr);
  if (path_chars == nullptr) {
    return nullptr;
  }
