...
from hokoff_model.live import NativeMaskProvider, masked_argmax, canonical_position_to_native
...
class MatchAgent:
    ...
    def prepare(self,raw_state,decks,*,batch_device=None,normalized_state=None):
        state = normalize_native_state(raw_state) if normalized_state is None else normalized_state
        if state.tick != raw_state['tick']: raise ValueError('prepared frame identity mismatch')
        self._prepared_frame = (raw_state, state)
        tick = state.tick
        ...
        batch = {key:value.to(self.device if batch_device is None else batch_device) for key,value in batch.items()}
        return batch,encoded,tick

    def decide(self,raw_state,decks,env):
        prepared=self.prepare(raw_state,decks)
        if prepared is None:return None,dict(policy_skipped=True,tick=raw_state['tick'])
        with torch.inference_mode(): output,hidden=self.model.forward_stream(prepared[0],self.hidden)
        return self.finish(prepared,output,hidden,raw_state,decks,env)

    def finish(self,prepared,output,hidden,raw_state,decks,env,*,batch_validated=False):
        batch,encoded,tick=prepared
        if self._prepared_frame is None or self._prepared_frame[0] is not raw_state:
            raise ValueError('finish must use the prepared immutable frame')
        state=self._prepared_frame[1]
        if not batch_validated and not all(bool(torch.isfinite(value).all()) for value in [*output.values(),*hidden]):
