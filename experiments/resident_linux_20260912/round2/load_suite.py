...
def native(which,op,count):
    script=ROOT/'launch_control.py' if which=='baseline' else ROOT/'launch_opt.py'
    cmd=[sys.executable,str(script),op,'--count',str(count)]
    if which!='baseline':cmd.append('--cache')
    subprocess.run(cmd,check=True,timeout=240)

def run(index,variant,count=12,rounds=2):
    native(variant,'start',count)
    out=ROOT/f'load-{index}-{variant}';out.mkdir();barrier=out/'go'
    jobs=[];samples=[]
    try:
        for i in range(count):
            result=out/f'policy-{i}.json';ready=out/f'ready-{i}.json'
            cmd=[sys.executable,str(ROOT/'run_variant.py'),'--port',str((24431 if variant=='baseline' else 25431)+i),
            ...
            with (out/f'policy-{i}.log').open('w') as log:
                p=subprocess.Popen(cmd,stdout=log,stderr=log,env={**os.environ,'CR_OPT_VARIANT':variant})
...
if __name__=='__main__':
    native('baseline','stop',24);native('dense','stop',24)
    results=[]
    try:
        for i,variant in enumerate(('baseline','dense','dense','baseline')):
            result=run(i,variant)
            if results:assert result['games']==results[0]['games'],'paired game divergence'
            results.append(result)
            (ROOT/'load-summary.json').write_text(json.dumps(results,indent=2))
            print(json.dumps({k:v for k,v in result.items() if k!='games'}),flush=True)
        (ROOT/'load-done.json').write_text(json.dumps(dict(complete=True,time=time.time())))
    finally:
        native('baseline','stop',24);native('dense','stop',24)
