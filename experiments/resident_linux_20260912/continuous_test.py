...
def run(model,encoder,c,args,mode,seed):
    ...
    begin=time.perf_counter();finals=[];peak_entities=0;action_hash=hashlib.sha256()
    next_game=args.slots;game_ids={i:i for i in active}
    game_seed=lambda g:seed+(g//args.slots)*1000+g%args.slots
    game_hashes={i:hashlib.sha256() for i in active};game_plays={i:0 for i in active}
    for cycle in range(2200*((args.games+args.slots-1)//args.slots)):
        ...
        for r in results:
            ...
            action_hash.update((json.dumps((i,game_ids[i],before,actions[i]),sort_keys=True)+'\n').encode())
            game_hashes[i].update((json.dumps((before,actions[i]),sort_keys=True)+'\n').encode());game_plays[i]+=len(actions[i])
            states[i]=envs[i]._enrich_state(r['state'])
            if done:
                finals.append(dict(slot=i,game=game_ids[i],seed=game_seed(game_ids[i]),tick=after,outcome=ep['outcome'],crowns=ep['crowns'],plays=game_plays[i],action_digest=game_hashes[i].hexdigest()))
                progress_path=(ROOT/args.output).with_suffix('.progress.json')
                temporary=progress_path.with_suffix('.tmp')
                temporary.write_text(json.dumps(dict(complete_games=len(finals),target_games=args.games,ticks=ticks,plays=plays,seconds=time.perf_counter()-begin)))
                os.replace(temporary,progress_path)
                if next_game<args.games:
                    config=copy.deepcopy(replay);config['rndSeed']=game_seed(next_game)
                    states[i]=envs[i].reset(config);game_ids[i]=next_game;next_game+=1
                    game_hashes[i]=hashlib.sha256();game_plays[i]=0
                    for agent in agents[i]+refs[i]:agent.reset(12)
    ...
    result=dict(device=args.device,mode=mode,raw_response=args.raw,slots=args.slots,seed=seed,seconds=wall,ticks=ticks,tps=ticks/wall,
                ...
                times=times,finals=finals,games_completed=len(finals),complete=len(finals)==args.games,peak_entities=peak_entities,
                continuous_refill=True,action_digest=action_hash.hexdigest())
...
def main():
    ...
    p.add_argument('--local-policy',action='store_true')
    ...
    a=p.parse_args();assert a.games>=a.slots;torch.set_num_threads(1);torch.set_num_interop_threads(1)
    torch.backends.cuda.matmul.allow_tf32=False;torch.backends.cudnn.allow_tf32=False
    if a.local_policy:
        a.device='cuda';model,encoder,identity=load_release(a.checkpoint,contract_path=a.contract,device='cuda')
    else:
        a.device='cpu';model,encoder,identity=load_release(a.checkpoint,contract_path=a.contract,device='cpu')
    ...
    try:
        ...
        c.close()
        if not a.local_policy:model.close()
