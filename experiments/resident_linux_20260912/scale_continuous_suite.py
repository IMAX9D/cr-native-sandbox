...
def tier(count,rounds,checkpoint,contract,games,local_policy):
    ...
    try:
        for i in range(count):
            ...
            cmd=[sys.executable,str(ROOT/'continuous_test.py'),'--port',str(BASE_PORT+i),'--raw','--device','cuda',
                 '--modes',','.join(['packed']*rounds),'--seed',str(424242+i*4),'--seed-stride','1000','--games',str(games),
                 '--timeout','900','--barrier',str(barrier),'--ready-file',str(ready),'--output',str(result),
                 '--checkpoint',checkpoint,'--contract',contract]
            if local_policy:cmd.append('--local-policy')
        ...
        deadline=time.monotonic()+rounds*910;next_sample=time.monotonic()+2
        ...
        result=dict(workers=count,resident_games=count*4,model_copies=count if local_policy else 1,policy_backend='per_worker_gpu' if local_policy else 'shared_gpu',rounds=rounds,
                    complete_games=sum(r['games_completed'] for r in rows),continuous_refill=True,wall_seconds=wall,ticks=sum(r['ticks'] for r in rows),
...
def main():
    ...
    p.add_argument('--label',default='shared')
    ...
    for n in map(int,a.counts.split(',')):
        if n not in (1,2,4,8,12,16):raise ValueError('unapproved tier')
        results.append(tier(n,a.rounds,a.checkpoint,a.contract,a.games,a.local_policy))
